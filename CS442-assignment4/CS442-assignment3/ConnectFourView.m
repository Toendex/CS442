...
- (id)initWithFrame:(CGRect)frame rows:(NSInteger)rows cols:(NSInteger)cols slotDiameter:(CGFloat)diameter
{
    ...
    if (self) {
        ...
        _boardView=[[BoardView alloc]initWithFrame:self.bounds rows:_rows cols:_cols slotDiameter:_slotDiameter];
...
- (void)addToColumn:(NSUInteger)column row:(NSUInteger)row withColor:(CGColorRef)color withAnimate:(BOOL)ifDoAnimate
{
    ...
    if(ifDoAnimate) {
        PieceView *piece=[[PieceView alloc] initWithFrame:CGRectMake((column+1)*width-_slotDiameter/2.0, -_slotDiameter,
                                                                     _slotDiameter, _slotDiameter) color:color];
        ...
        PieceView *piece=[[PieceView alloc] initWithFrame:CGRectMake((column+1)*width-_slotDiameter/2.0, (_rows-row)*height-_slotDiameter/2.0,_slotDiameter, _slotDiameter) color:color];
        [_pieceViews addObject:piece];
        [self insertSubview:piece belowSubview:_boardView];
    }
}
